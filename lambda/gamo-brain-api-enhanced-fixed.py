    ...
    try:
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                # Save to DynamoDB (convert floats to Decimals)
                table.put_item(
                    Item={
                        'pk': 'embedding',
                        'sk': chunk_id,
                        ...
                        'documentId': document_id,
                        'text': chunk,
                        'embedding': [Decimal(str(f)) for f in embedding],
...
def upsert_to_pinecone(vector_id: str, embedding: List[float], metadata: Dict[str, Any]):
    """Upsert vector to Pinecone using REST API"""
    if not PINECONE_API_KEY:
        return
    
    try:
        # Get index host from control plane first time
        # For serverless indexes, host format is: {index-name}-{id}.svc.{environment}.pinecone.io
        # We'll use the API to get the correct host
        host_url = get_pinecone_host()
        if not host_url:
            print('Could not get Pinecone host')
            return
        
        url = f'https://{host_url}/vectors/upsert'
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            print(f'Pinecone upsert success: {result.decode()}')
    
    except Exception as e:
        print(f'Pinecone upsert exception: {str(e)}')

def get_pinecone_host():
    """Get the correct Pinecone host from control plane"""
    try:
        req = urllib.request.Request(
        ...
        print(f'Failed to get Pinecone host: {str(e)}')
        return None

def query_pinecone(embedding: List[float], top_k: int = 5) -> List[Dict[str, Any]]:
    """Query Pinecone for similar vectors using REST API"""
    if not PINECONE_API_KEY:
        return []
